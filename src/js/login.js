  ...
  try {
    ...
    if (data.role === 'admin') {
      ...
      try {
        ...
        const votingToken = await instance.generateVotingToken({ from: account });

        await instance.registerVoter(votingToken, account, { from: account });

        localStorage.setItem('votingToken', votingToken);

        // Send voting token to backend to generate and send QR code
        const qrResponse = await fetch('http://192.168.1.6:8000/send-voting-token', {
          ...
          body: JSON.stringify({ voter_id: voter_id, voting_token: votingToken })
        });
        ...
        console.error('Failed to interact with smart contract:', error.message);
      }
    }
  } catch (error) {
